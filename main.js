...
function drawWheel(canvas) {
  ...
  const radius = Math.min(cx, cy) - 20; // ⚙️ Трохи збільшено відступ, щоб уникнути обрізання
  ...
  switch (mode) {
    case "desktop":
      fontScale = 0.12; // ⚙️ Розмір тексту на desktop
      imgScale = 0.26; // ⚙️ Розмір картинок на desktop
      textYOffset = 0.82; // ⚙️ Відстань тексту від центру (більше = далі)
      imgYOffset = 0.56; // ⚙️ Відстань картинки від центру
      break;
    case "tablet":
      fontScale = 0.105;
      imgScale = 0.13;
  ...
  ctx.fillStyle = "#000"; // ⚙️ Колір фону колеса
  ctx.fill();

  // Рамка колеса
  ctx.beginPath();
  ctx.arc(cx, cy, radius, 0, Math.PI * 2);
  ctx.lineWidth = radius * 0.05; // ⚙️ Товщина рамки колеса
  ctx.strokeStyle = "#ebc566"; // ⚙️ Колір рамки колеса (золотий)
  ctx.shadowColor = "#FFD700"; // ⚙️ Колір тіні рамки (золотий)
  ctx.shadowBlur = radius * 0.05; // ⚙️ Розмиття тіні рамки
  ctx.stroke();
  ...
  prizes.forEach((p, i) => {
    ...
    ctx.fillStyle = "#fff"; // ⚙️ Колір тексту (білий)
    ctx.strokeStyle = "#000"; // ⚙️ Колір обводки тексту (чорний)
    ctx.lineWidth = fontSize * 0.1; // ⚙️ Товщина обводки тексту
  ...
  for (let i = 0; i < prizes.length; i++) {
    ...
    // Темна обводка
    ctx.strokeStyle = "#e7c710ff"; // ⚙️ Колір обводки розділювача (коричневий)
    ctx.lineWidth = radius * 0.025; // ⚙️ Товщина обводки розділювача
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ...
    // Золота лінія
    ctx.strokeStyle = "#FFD700"; // ⚙️ Колір розділювача (золотий)
    ctx.lineWidth = radius * 0.015; // ⚙️ Товщина розділювача
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.lineTo(endX, endY);
    ctx.stroke();
